...
## 10. v1.3 架构调整评估（猫咪后院 2D + 自适应 + 猫角色）

| 层 | 调整 | 说明 |
|---|---|---|
| 视口 | 固定 1920×1080 → **窗口自适应**（resize 实时重设画布与相机视口；室内缩放因子按窗口重算；超大屏视口大于世界时底图居中） | 连续坐标世界天然支持任意视口，改动只在视口层 |
| 风格 | **伪3D 退化为扁平 2D**：删除 depthScale 纵深缩放与"靠近建筑放大"，建筑/角色固定尺寸 | 是 v1.2 引擎的子集，做减法即可；保留 y 排序遮挡与阴影（猫咪后院也有前后关系） |
| 角色 | `drawCharacter`（Q 萌小人）→ **`drawCat`**：四方向头部画法（正/背/侧）+ 两腿交替迈步（正背=左右腿上下倒腾，侧面=前后腿倒腾）+ 尾巴持续摆动；主角配色复刻 user.png，NPC 为不同花色猫 | **user.png 为单帧立绘，无法直接逐帧动画**——这是评估结论；原图用于登录页/护照头像。若要原画动画需 4 方向 × 2 帧精灵图，引擎可直接换 |
| 素材管线 | 新增 `scripts/cut-buildings.js`（npm run assets）：边缘洪水填充抠渐晕背景（容差吸收噪点 + 斑点清理 + 连通块过滤）；棋盘格"假透明"图取边缘双主色做色键填充 | 用户给的立绘除 book-plan2 外均带不透明背景，必须预处理；输出 ui-building/cut/ |
| 交互 | 进建筑从"踩门自动传送"→ **门口提示 + 确认**（按 E 或点击）；点击建筑寻路到门后视为已确认直接进入；室内出门保持自动 | 满足"有可交互的进入提示就行" |
| HUD | 删除小地图导航及其渲染循环 | — |

结论：v1.3 是 v1.2 引擎的**简化**（去纵深）+ 角色绘制层替换 + 素材预处理管线，核心架构第三次完整复用。

## 13. v1.7 架构调整（遮挡多边形裁剪重绘）

- **抠图路线退役**：sprite 抠图无法保证零残留（与建筑连通的草地/云难以根除），残留像素会在玩家位于建筑后方判定时遮挡玩家。
- **裁剪重绘法**：底图整张绘制 → 角色按深度排序绘制 → 每栋建筑的"遮挡 entity"用 `ctx.clip(遮挡多边形)` 把底图原位重绘一遍。多边形 6~7 个顶点手工沿建筑轮廓内缩标定，顶点全在建筑实体内：遮挡范围内不存在背景像素，正确性由几何保证而非图像处理效果。
- **细节**：玩家头顶可达高度（可走区 minY − 猫身高）以上的屋顶装饰（咖啡杯、卫星锅、铁塔）无需纳入多边形；建筑点击命中改为多边形射线法判定；换底图只需重标多边形顶点，无需任何图像预处理。

## 12. v1.6 架构调整（整图布局 + sprite 原位贴回）

- **美术工作流反转**：不再由代码摆放独立建筑立绘，而是以用户排版好的整图为基准；`scripts/extract-buildings.js` 从整图裁出四栋建筑并抠背景（边缘色域洪水填充 + 建筑核心保护区防穿透 + 斑点清理 + 最大连通块过滤），输出 ui-building/v1.6/sprites/。
- **图层原理**：底图整张绘制（建筑像素仍在底图上），抠出的 sprite 在完全相同位置贴回，与角色一起按深度（建筑取主体底边 anchorY）排序绘制——sprite 像素与底图逐像素对齐，视觉无缝；角色在 anchorY 之上时被 sprite 盖住，之下时盖住 sprite。
- **点击命中**：建筑 sprite 预绘到离屏 canvas，点击时取该点 alpha（>40 才算命中），重叠时取 anchorY 最大者，与渲染遮挡一致。
- **逐栋手工标定**：裁剪框 box、深度 anchorY、门口圈 door、占地椭圆 fp 均按布局图标定（不再有统一公式）。

## 11. v1.4 架构调整（固定场景）

- **删除相机系统**：户外改为固定逻辑坐标系（1672×941，与岛图等比），渲染层按窗口 cover 缩放铺满并居中（超宽/超窄屏裁切边缘，建筑都在安全区）。固定场景与屏幕自适应（v1.3）同时成立，resize 不需要迁移任何实体坐标。
- **素材管线退役**：ui-building/new/ 全部为真透明 PNG，直接使用；删除抠图脚本与 pngjs 依赖、cut/ 产物。
- **比例调整**：建筑 renderW 约 300（场景宽 18%），猫 CAT_SCALE 2.1（屏高约 14%），贴合猫咪后院物件比例。
